...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True, show_BB=True):
    ...
    colors = list(((255, 0, 0), (0, 0, 255), (255, 0, 255)))
...
def draw_bbox_tracker(image, objects, rects, colors, disappeared, BoundinBox, totalObjetos = [], fps = [], frame = [], trackerType = []):
    image_h, image_w, _ = image.shape
    corBox = (255, 255, 255)
    ...
    if trackerType != []:
        texto = "Tracker Type: " + trackerType
        cv2.putText(image, texto, (15, 230),
            cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 0, 0), 2)
